...
class TrainingLogger:
    ...
    def __init__(self, root_dir: str | Path = "Results", summary_interval: int = 50):
        ...
        self.success_csv_path = Path(root_dir) / "episode_success.csv"
        self.summary_csv_path = self.run_dir / "summary_metrics.csv"
        self.update_csv_path = self.run_dir / "ppo_update_metrics.csv"
        self.reward_weight_csv_path = self.run_dir / "reward_weight_metrics.csv"
        ...
        self._reward_weight_rows: list[dict[str, Any]] = []
    ...
    def log_reward_weights(
        ...
        row_dict = {
            ...
            "episode": episode,
            "step": step,
            "w_goal_progress": weights[0],
            "w_goal_proximity": weights[1],
            "w_obs_dist": weights[2],
            "w_apf": weights[3],
            "weighted_reward": weighted_reward,
            "task_reward": task_reward,
            "reward_update_loss": self._metric_value(update_metrics, "loss"),
            "reward_update_corr": self._metric_value(update_metrics, "corr"),
            "reward_buffer_size": self._metric_value(update_metrics, "buffer_size"),
        }
        self._reward_weight_rows.append(row_dict)
        self._append_csv_row(
            self.reward_weight_csv_path,
            [
                row_dict["global_step"],
                row_dict["episode"],
                row_dict["step"],
                row_dict["w_goal_progress"],
                row_dict["w_goal_proximity"],
                row_dict["w_obs_dist"],
                row_dict["w_apf"],
                row_dict["weighted_reward"],
                row_dict["task_reward"],
                row_dict["reward_update_loss"],
                row_dict["reward_update_corr"],
                row_dict["reward_buffer_size"],
            ],
        )

        if self.writer:
            self.writer.add_scalar("reward_weight/w_goal_progress", weights[0], global_step)
            self.writer.add_scalar("reward_weight/w_goal_proximity", weights[1], global_step)
            self.writer.add_scalar("reward_weight/w_obs_dist", weights[2], global_step)
            self.writer.add_scalar("reward_weight/w_apf", weights[3], global_step)
            self.writer.add_scalar("reward/weighted", weighted_reward, global_step)
            self.writer.add_scalar("reward/task_discrete", task_reward, global_step)
            if update_metrics:
                self.writer.add_scalar("reward_weight_update/loss", update_metrics["loss"], global_step)
                self.writer.add_scalar("reward_weight_update/corr", update_metrics["corr"], global_step)
    ...
    def log_episode(
        ...
        self._append_csv_row(
            ...
            [
                row_dict["episode"],
                self._episode_result_label(metrics.arrived, metrics.collided, metrics.timeout),
                row_dict["arrived"],
                row_dict["collided"],
                row_dict["timeout"],
                row_dict["steps"],
    ...
    @staticmethod
    def _episode_result_label(arrived: bool, collided: bool, timeout: bool) -> str:
        if arrived:
            return "arrived"
        if collided:
            return "collided"
