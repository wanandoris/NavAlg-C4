...
class RewardConfig:
    reward_arrive_bonus: float = 8
    reward_collision_penalty: float = -8
    reward_near_target_bonus: float = 1
    reward_weight_distance: float = 1.2
    reward_weight_obstacle: float = 0.7
    reward_weight_heading: float = 0.8
    reward_weight_time: float = 0.05
    ...
    obstacle_reward_min: float = -2.5
    obstacle_reward_max: float = 1.0
    heading_reward_min: float = -1.5
    heading_reward_max: float = 1.0
    step_reward_min: float = -3.0
    step_reward_max: float = 3.0
...
def compute_reward_breakdown(
    ...
    progress_raw = calc_progress_reward(prev_distance, current_distance, config)
    proximity_raw = calc_proximity_reward(current_distance, max_distance, config)
    distance_reward_raw = calc_distance_reward(current_distance, prev_distance, max_distance, config)
    ...
    obstacle_reward_raw = _clip(
        -calc_repulsive_potential(obstacle_min_range, config),
        config.obstacle_reward_min,
        config.obstacle_reward_max,
    )
    if current_distance < config.target_slow_range:
        obstacle_reward_raw = config.reward_near_target_bonus
    heading_reward_raw = _clip(heading_reward_raw, config.heading_reward_min, config.heading_reward_max)
    ...
    reward = _clip(
        distance_reward + obstacle_reward + heading_reward + time_reward + config.step_penalty,
        config.step_reward_min,
        config.step_reward_max,
    )
    if arrive:
        reward += config.reward_arrive_bonus * calc_time_reward_weight(episode_elapsed_time, config)
    elif done:
...
def calc_distance_reward(
    ...
    return (
        config.progress_reward_weight * calc_progress_reward(prev_distance, current_distance, config)
        + config.proximity_reward_weight * calc_proximity_reward(current_distance, max_distance, config)
    )
...
def calc_time_reward_weight(
    ...
    if episode_elapsed_time is None or config.max_episode_time <= 0:
        return 1.0
    progress = max(0.0, min(1.0, episode_elapsed_time / config.max_episode_time))
...
def calc_apf_heading_reward(
    ...
    turn_action = _clip_turn_action(_extract_turn_action(action))
    ...
    return round((curr_align - prev_align) * 2.0, 2)
...
def calc_apf_heading_diff(
    ...
    if rho < rho_0:
        ...
        obstacle_world_deg = heading_world + float(obstacle_angle)
