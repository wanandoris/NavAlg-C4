...
class SAC_NAV:
    ...
    def __init__(self, ros_ctrl: Ros2Controller, global_data: GlobalData, xyzAxis=True):
        ...
        #模型
        self.ppo_agent = SAC(ifload=False,file_path = "D:\\大赛资源\\智能导航C4-2026\\unpack\\NavAlg-C4-v1\\sac_%s.pt"%(str(LOAD_MODEL_STEP)))
    ...
    def navigationHandler(self,state,episode,step,global_step):
        ...
        try:
            ...
            action= self.ppo_agent.run(state,self.reward,self.done or self.arrive,global_step)
